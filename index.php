...
if (!isset($_SESSION['user_id']) && isset($_COOKIE['login_token'])) {
    $token = htmlspecialchars($_COOKIE['login_token']);  // Sanitizing input
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($user)) {
    setcookie('email', htmlspecialchars($user['email']), time() + (86400 * 30), "/"); // 30 Tage gültig
}
?>
<!DOCTYPE html>
<html lang="de">
...
<?php
$page = htmlspecialchars($_GET['page'] ?? 'home'); // Sanitizing input
